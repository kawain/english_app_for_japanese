				...
				for i, line := range lines {
					...
					if len(fields) == 6 {
						for j := range fields {
							...
							// バックスラッシュを削除
							fields[j] = strings.ReplaceAll(fields[j], "\\", "")
						}
						if fields[0] == "" {
							consoleLog.Invoke(js.ValueOf(fmt.Sprintf("Go関数(InitializeAppData): IDが空のため %d 行目をスキップします: %s", i+1, line)))
							continue
						}
						obj := objects.NewDatum(fields[0], fields[1], fields[2], fields[3], fields[4], fields[5])
						appData.AddData(obj)
						addedCount++
					} else if len(strings.TrimSpace(line)) > 0 {
						consoleLog.Invoke(js.ValueOf(fmt.Sprintf("Go関数(InitializeAppData): 不正な行 %d をスキップします (期待されるフィールド数: 7, 実際のフィールド数: %d): %s", i+1, len(fields), line)))
					}
				}
...
// SearchData はJavaScriptから呼び出され、データを検索し、
// シャッフルされた結果をJavaScriptのオブジェクト配列として返します。
//
// 引数:
//   - args[0]: level (数値型)
//   - 0: ローカルストレージに含まれないデータを検索
//   - 1: ローカルストレージ（学習済みなど）に含まれるデータを検索
//
// 戻り値:
...
func SearchData(this js.Value, args []js.Value) any {
	handler := js.FuncOf(func(this js.Value, promiseArgs []js.Value) interface{} {
		...
		go func() {
			...
			default:
				reject.Invoke(js.ValueOf("Go関数(SearchData)エラー: 引数は0または1である必要があります"))
				return
			}
...
func SearchWord(this js.Value, args []js.Value) any {
	handler := js.FuncOf(func(this js.Value, promiseArgs []js.Value) interface{} {
		resolve := promiseArgs[0]
		reject := promiseArgs[1]
		go func() {
			if appData.Data == nil {
				// InitializeAppDataが完了していないか、失敗した可能性
				reject.Invoke(js.ValueOf("Go関数(SearchWord)エラー: appDataが初期化されていません。InitializeAppDataが正常に完了したか確認してください。"))
				return
			}
			if len(args) != 1 {
				reject.Invoke(js.ValueOf("Go関数(SearchWord)エラー: 引数は1つ必要です"))
				return
			}
			if args[0].Type() != js.TypeString {
				reject.Invoke(js.ValueOf("Go関数(SearchWord)エラー: 引数は文字列型である必要があります"))
				return
			}
			...
			for _, v := range appData.Data {
				if strings.Contains(v.Word, keyWord) {
					results = append(results, v)
				}
			}

			consoleLog.Invoke(js.ValueOf("Go関数(SearchWord)で検索したデータの長さ:"), js.ValueOf(len(results)))
			// --- JavaScriptのデータに変換 ---
			jsResult := make([]interface{}, len(results))
			for i, v := range results {
				obj := map[string]interface{}{
