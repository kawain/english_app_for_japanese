...
// CreateQuiz はJavaScriptから呼び出され、指定された選択肢の数に基づいて
// 新しいクイズの問題（正解データ）を準備し、そのデータを返します。
//
// 内部で quizData の初期化または更新、次の問題への遷移、選択肢の生成を行います。
//
// 引数:
//   - args[0]: choiceCount (数値型) - 生成する選択肢の数（正解を含む）。
//
// 戻り値:
...
func CreateQuiz(this js.Value, args []js.Value) any {
	handler := js.FuncOf(func(this js.Value, promiseArgs []js.Value) interface{} {
		...
		go func() {
			...
			choiceCount := args[0].Int()
			// もしもquizDataにQuizDataがない
			if quizData.FilteredArray == nil {
				quizData.Init(&appData, choiceCount)
			}
			// 次の問題へ(最初の問題含む)
			quizData.Next()
